...
void PFSPrint_long_dir_entry(LongDirEntry* long_dir_entry) {
	...
	printf(" (UTF16)");
	ln();/*
	 Aux_printf("name_1+2+3: ");
	 for (i = 0; i < sizeof(len_utf8_name1); i++)
	 Aux_printf("%1c", utf8_name1[i]);
	 for (i = 0; i < sizeof(len_utf8_name2); i++)
	 Aux_printf("%1c", utf8_name2[i]);
	 for (i = 0; i < sizeof(len_utf8_name3); i++)
	 Aux_printf("%1c", utf8_name3[i]);
	 Aux_printf(" (UTF8)");
	 ln();*/
...
void PFSPrint_FAT_table() {
	...
	ln();
	printf("> FAT Table\n");
	while (FAT_table->entry[i] != FREE_CLUSTER) {
		printf("Entrada %d: %x\n", i, FAT_table->entry[i]);
		i++;
	}
}
...
void PFSPrint_fs_info() {
	...
	for (i = 0; i < sizeof(fs_info->reserved); i++)
		Aux_printf("%d ", fs_info->reserved[i]);
	ln();
	...
	for (i = 0; i < sizeof(fs_info->reserved2); i++)
		Aux_printf("%d ", fs_info->reserved2[i]);
	ln();
	Aux_printf("sector_signature3: %s\n", fs_info->sector_signature3);
}
...
void PFSPrint_block(Block block, int n_block) {
	...
	for (i = 0; i < config_pfs.sectors_per_block; i++) {
		PFSPrint_sector(block.sector[i], n_block * config_pfs.sectors_per_block + i);
	}
}

void PFSPrint_cluster(Cluster cluster, uint32_t n_cluster) {
	int i, j;
	ln();
	Aux_printf("> Cluster %u\n", n_cluster);
	for (j = 0; j < config_pfs.blocks_per_cluster; j++)
		for (i = 0; i < config_pfs.sectors_per_block; i++) {
			PFSPrint_sector(cluster.block[j].sector[i], bs->reserved_sector_count + (bs->number_of_fats * bs->fat32_sectors_per_fat) + (n_cluster * bs->sectors_per_cluster) + i);
		}
}

void PFSPrint_sector(Sector sector, int n_sector) {
	int i;
	//Aux_printf(">> Sector %u: ", n_sector);
	for (i = 0; i < config_pfs.bytes_per_sector; i++)
		printf("%x", sector.byte[i]);
	ln();
}

void PFSPrint_config_pfs(void) {
	Aux_printf("> Config PFS: ");
	Aux_printf("%d ", config_pfs.bytes_per_sector);
	Aux_printf("%d ", config_pfs.blocks_per_cluster);
	ln();
}
