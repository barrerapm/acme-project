...
void FAT_get_file_clusters(BootSector* bs, TablaFAT* FAT_table, int* file_clusters) {
	int total_entries = (bs->bytes_per_sector * bs->sectors_per_cluster) / ROOT_ENTRY_BYTES;
	int n_entry = file_clusters[0] + bs->fat32_cluster_num_dir_start;
	int n = 1;
...
int FAT_get_free_clusters(BootSector* bs, FSInfo* fs_info, TablaFAT* FAT_table, int n_free_clusters, int* free_clusters) {
	...
	int n = 0, first_free_cluster = fs_info->most_recently_alloc_cluster - bs->fat32_cluster_num_dir_start + 1;
	...
	while ((n_free_clusters > n) && (n_cluster < last_entry)) { // Mientras no llegue a la cantidad de clusters pedidos y no llegue al final de la FAT
		n_cluster++;
		if (FAT_table->entry[n_cluster + bs->fat32_cluster_num_dir_start] == FREE_CLUSTER) {
...
void FAT_write_FAT_entry(BootSector* bs, FSInfo* fs_info, TablaFAT* FAT_table, int n_free_clusters, int* free_clusters) {
	...
	for (i = 0; i < n_free_clusters - 1; i++) {
		n_entry = free_clusters[i] + bs->fat32_cluster_num_dir_start;
